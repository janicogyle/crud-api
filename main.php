...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        ...
        if($req[0]=='GetAllPeople'){echo json_encode($try->getAll()); return;}
        if ($req[0] == 'Get_Single_People' && isset($req[1])) {$userId = $req[1];echo json_encode($try->getUserById($userId));return; }
        break;
    case 'POST':
        $data_input = json_decode(file_get_contents("php://input"));
        if($req[0] == 'insert_people'){echo json_encode($try->insert_people($data_input)); return;}
        break;

    case 'PUT':
        if ($req[0] == 'udpate_people' && isset($req[1])) { $userId = $req[1]; 
    ...
    case 'DELETE':
        if ($req[0] == 'delete_people' && isset($req[1])) {
            $userId = $req[1]; 
